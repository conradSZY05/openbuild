...
export default function Projects() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (!error && data) {
        ...
        // fetch all profiles for project posters
  ...
  const getAvatar = (userId, avatarUrl) => {
      ...
      <div style={{ marginRight: '280px' }} className="flex-1">
        ...
        <div className="flex justify-between items-center mb-3">
          <a href="/projects" className="hover:opacity-70">
            <img src="/logo.png" alt="OpenBuild" style={{ height: '75px' }} />
          </a>
        ...
        <div className="flex gap-2 mb-3">
          <input
            ...
            className="border border-stone-400 px-3 py-1 text-sm text-stone-800 bg-stone-100 focus:outline-none w-64"
          />
          <select
            value={skillFilter}
          ...
          {filtered.map((project, index) => {
            ...
            return (
              ...
              >
                ...
                {profile?.university && (
                  <p className="text-xs text-stone-500 mt-1 leading-tight">{profile.university}</p>
                )}
                {profile?.course && (
                  <p className="text-xs text-stone-400 leading-tight">{profile.course}</p>
                )}
                <p className="text-xs text-stone-500 mt-1">
                  Rep: <span className="font-bold" style={{ color: '#2c4a7c' }}>{profile?.reputation || 0}</span>
                </p>
              </div>

                {/* Right — project content */}
                <div className="px-4 py-3 border-l border-stone-400" style={{ minWidth: 0, flex: 1 }}>
                  <p className="font-semibold text-base hover:underline" style={{ color: '#2c4a7c' }}>
                    {project.title}
                  </p>
